...
nvs50plus <- nvs2018[which(nvs2018$AGECAT == "50-64 years" | nvs2018$AGECAT == "65+ years"), ]

##### How helpful are the information sources?
infoTitle <- "How helpful are the information sources?"

infoItems50 <- subset(nvs50plus, select = c(PREVKNOW:OTHERWEB, SOCMINFO:OTHERSOURCE))
...
## Other Website
otherWeb <- nvs50plus$OTHERWEBTEXT
otherWeb <- otherWeb[!is.na(nvs50plus$OTHERWEBTEXT)]
otherWeb

## Other Source
otherSource <- nvs50plus$OTHERSOURCETEXT
otherSource <- otherSource[!is.na(nvs50plus$OTHERSOURCETEXT)]
otherSource
